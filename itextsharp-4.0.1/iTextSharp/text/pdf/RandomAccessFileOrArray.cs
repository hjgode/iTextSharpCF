...
using System.Net;
/*
 * $Id: RandomAccessFileOrArray.cs,v 1.7 2006/10/06 15:06:57 psoares33 Exp $
 * $Name:  $
 *
        ...
        public RandomAccessFileOrArray(String filename, bool forceRead) {
            if (!File.Exists(filename)) {
                if (filename.StartsWith("file:/") || filename.StartsWith("http://") || filename.StartsWith("https://")) {
                    Stream isp;
#if !NETCF
                    isp = WebRequest.Create(new Uri(filename)).GetResponse().GetResponseStream();
#else
                    if (filename.StartsWith("file:/"))
                        isp = new FileStream(filename.Substring(6), FileMode.Open);
                    else
                        isp = WebRequest.Create(new Uri(filename)).GetResponse().GetResponseStream();
#endif
                    try {
...
#else
            if(url.IsFile)
                isp = new FileStream(url.LocalPath, FileMode.Open);
            else
                isp = WebRequest.Create(url).GetResponse().GetResponseStream();
#endif
            try {
